...
The widget generates a number of events that you can listen for in order to react to user input.  The most important of these is the `click` event.
This is generated by clicking an item with the mouse, hitting space or enter when an item is highlighted, or when a global keyboard shortcut is detected.

The following code snippet will display the ID of the clicked item (assuming that the `menu` variable already contains a valid Menu instance):

```javascript
menu.addEventListener('click', function (e) {
	alert('Menu item with ID ' + e.detail +' was clicked');
});
```

All events triggered by the widget include the ID of the clicked item in the `detail` property of the event object passed to the handler.
